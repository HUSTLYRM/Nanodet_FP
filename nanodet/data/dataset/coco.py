...
# 使用 TODO 标注重要的内容
# get_per_img_info      获取图片信息
# get_img_annotation    获取标注信息!
...
class CocoDataset(BaseDataset):
    ...
    def get_data_info(self, ann_path):
        ...
        """
        self.coco_api = COCO(ann_path)                      # cocoapi
    ...
    def get_img_annotation(self, idx):
        ...
        if self.use_instance_mask:
            gt_masks = []
        # TODO: 添加use_keypoint, 然后就会从中获取ketpoint的信息
        if self.use_keypoint:
            gt_keypoints = []
        for ann in anns:
            x1, y1, w, h = ann["bbox"]
            if ann["area"] <= 0 or w < 1 or h < 1:          # 判断 area 以及图片的 w h
                continue
            if ann["category_id"] not in self.cat_ids:      # 判断类别
                continue
            # 添加的是 x1, y1, x2, y2 两个角点
        ...
        if self.use_keypoint:
            if gt_keypoints:                                                            
                annotation["keypoints"] = np.array(gt_keypoints, dtype=np.float32)  # keypoint的信息添加到其中
            else:
                annotation["keypoints"] = np.zeros((0, 51), dtype=np.float32)       # 这里暂时用不到
        return annotation
